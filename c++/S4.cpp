...
int main(int argc, char *argv[])
{
  ...
  double rhoBout, rhoEout, SgEout, SgEpout, SgBout, SgBpout;
  ...
  while (true) {
    ...
    if (!(SgEout > 0) || !(SgEpout > 0) || !(SgBout > 0) || !(SgBpout > 0)) {
      SgEout = SgEin*SGSAFE;
      SgEpout = SgEpin*SGSAFE;
      SgBout = SgBin*SGSAFE;
      SgBpout = SgBpin*SGSAFE;

      cout << ", Sg is reduced to (SgEout, SgEpout, SgBout, SgBpout) = (" << SgEout << ", " << SgEpout << ", " << SgBout << ", " << SgBpout << ")" << flush;
    }
    cout << endl;

    if (abs(SgEin-SgEout)/SgEout < TOL && abs(SgEpin-SgEpout)/SgEpout < TOL && abs(SgBin-SgBout)/SgBout < TOL && abs(SgBpin-SgBpout)/SgBpout < TOL) {
      break;
    }

    SgEin = pow(SgEin*SgEin*SgEin*SgEout,1./4);
    SgEpin = pow(SgEpin*SgEpin*SgEpin*SgEpout,1./4);
